...
func assertChildren(t *testing.T, h *position.Hierarchy, parent entity.Entity, expect []entity.Entity) {
	t.Helper()
...
func TestNew(t *testing.T) {
	...
	t.Run("should return an error if a cyclic relationship is detected", func(t *testing.T) {
		...
		if err := h.Update(child3, child1); !errors.Is(err, position.ErrCyclicRelationship) {
	...
	t.Run("deleting a parent should remove the children from the hierarchy", func(t *testing.T) {
		...
		if _, err := h.Parent(child2); !errors.Is(err, position.ErrEntityNotFound) {
			t.Errorf("Child2 should not have a parent: %s", err)
		}

		if _, err := h.Parent(child3); !errors.Is(err, position.ErrEntityNotFound) {
			t.Errorf("Child3 should not have a parent: %s", err)
		}

		// check if the parent is removed from the hierarchy
		if _, err := h.Parent(child1); !errors.Is(err, position.ErrEntityNotFound) {
